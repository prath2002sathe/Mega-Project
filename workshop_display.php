...
<a href="workshop.php" class="btn btn-primary float-right btn-fill">ADD WORKSHOP</a>
 <!-- Search form -->
 <form action="workshop_display.php" method="GET">
    ...
    <thead class=" " style="background-color:#9775D9;font-size:0.9em;">
        ...
        <tbody class="list">
            ...
            if(isset($_GET['sdate']))
            {
              ...
              $ldate =  $_GET['ldate'];
                ...
                if(isset($_GET['search']))
                {
                        ...
                        if(isset($_GET['searchvalue'])){
                            if($_GET['searchvalue'] != ""){
                                ...
                                //check search , sdate last date 
                                    $sql="SELECT * FROM workshop INNER JOIN register WHERE workshop.register_id = register.register_id AND  register.register_dept = $register_dept  AND register.register_name LIKE '%".$searchvalue."%'";
                                   
                             }
                             else
                             {
                                 // sdate last date 
                                 $sql="SELECT * FROM workshop INNER JOIN register WHERE workshop.register_id = register.register_id AND  register.register_dept = $register_dept AND workshop.workshop_date BETWEEN '$sdate' AND '$ldate'";
                             }
                        }
                    ...
                    $sql="SELECT * FROM workshop INNER JOIN register WHERE workshop.register_id = register.register_id AND  register.register_dept = $register_dept";
                 }
            }
            else{
                 //no filtres are enabled 
            ...
            while($row = mysqli_fetch_array($result))
            {
                ...
                ?>
                <!-- style="display:<?php if($flag == 1){echo "hide";}else{echo "block";} ?>" -->
                <tr>
                <td><?php echo $i;?></td>
                <!-- <td><?php echo $row['register_name']; ?></td> -->
                <td><?php echo $row['workshop_name']; ?></td>
                <td><?php echo $row['workshop_topic']; ?></td>
                <td><?php echo $row['workshop_duration']; ?></td>
                <td><?php echo $row['workshop_organiser']; ?></td>
                <td><?php echo $row['workshop_date']; ?></td>
                <td><?php echo $row['workshop_resource']; ?></td>
                <td><a href="<?php echo $row['workshop_certificate']; ?>" target="_blank">View Document</a></td>
                <td><?php if($row['workshop_status'] == "1"){echo "Yes";}else{echo "No";} ?></td>
                <td><a href="<?php echo $row['workshop_status_receipt']; ?>" target="_blank">View Receipt</a></td>
            </tr>
            <?php
            }
        }
